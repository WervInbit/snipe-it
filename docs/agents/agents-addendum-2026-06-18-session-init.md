...
- Local commits prepared for remote testing: component QR/tag/serial-removal handling, inactivity timeout and UI follow-ups, and the current planning/session documentation.
- Do not treat the branch as production-verified yet. The next remote test pass should cover component QR print/scan, moved-component detail redirects, serial capture while removing to tray, inactivity timeout and explicit keepalive, selected workflow start on the asset Tests tab, Workflow Items Select2 controls, and Dutch UI wording.
