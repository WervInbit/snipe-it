...
@foreach($attachedChildRows as $childRow)
    @include('components.partials.asset-roster-row', ['row' => $childRow, 'asset' => $asset, 'depth' => 1])
@endforeach

@foreach($removedExpectedChildren as $child)
    @include('components.partials.asset-subcomponent-removed-row', ['child' => $child, 'asset' => $asset, 'depth' => 1])
@endforeach
...
@foreach($expectedSubcomponents as $template)
    ...
    @endphp
    @include('components.partials.asset-subcomponent-expected-row', [
